...
def feedback():
    ...
    for i in range(5): 
        ...
        if result[i] == 'y':
            _not[i+1].append(attempt[_round][i])
    ...
    for i in range(1,6): 
        ...
        if _answer[i] != '?': 
            ...
            for j in range(len(possible)-1,-1,-1): 
                ...
                if _answer[i] != possible[j][i-1]:
                    #print('{} has been removed'.format(_possible[j]))
                    _possible.remove(possible[j])
    for i in range(len(_yellow)):
        for j in range(len(possible)-1,-1,-1): 
                #print('\n _possible[{}][{}] = {}'.format(j,i-1,_possible[j][i-1]))
                if not _yellow[i] in possible[j]:
                    #print('{} has been removed'.format(_possible[j]))
                    possible.remove(possible[j])
...
# TEST
    ...
    if len(_green) + len(_yellow) > 2 or len(possible) < 100: 
        newpossible = complete(possible)
...
_round = new(_possible1)
feedback()

_possible2 = []
transferlist(possible, _possible2)
_round = new(_possible2)
feedback()
